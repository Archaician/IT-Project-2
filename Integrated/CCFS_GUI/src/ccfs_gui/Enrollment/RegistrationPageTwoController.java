...
public class RegistrationPageTwoController implements Initializable {
    ...
    private void addSiblingButton(ActionEvent event) throws IOException {
        DialogWindows.dialogBox(Alert.AlertType.CONFIRMATION, "Add Sibling", "WALA PA ITO.", ButtonType.OK, null, null);
    }
    @FXML
    private void enrollButton(ActionEvent event) throws IOException {
        String[] otherInfo = {fathername.getText(),fatheroccupation.getText(),
        fatheraddress.getText(),fatherworknum.getText(),fatherhomenum.getText()
        ,fathermobile.getText(),fatheremail.getText(),mothername.getText(),
        motheraddress.getText(),motheroccupation.getText(),
        motherworknum.getText(),motherhomenum.getText(),mothermobile.getText(),
        motheremail.getText(),guardianname.getText(),guardianaddress.getText(),
        guardiannumber.getText()};
        ImpleEn.addInfo(otherInfo);
//        ClientCon.stub.enStud(ImpleEn.studInform.getFirst()), ImpleEn.studInform.get(2),);
        DialogWindows.dialogBox(Alert.AlertType.CONFIRMATION, "Register Student", "Successfully registered student.", ButtonType.OK, null, null);
